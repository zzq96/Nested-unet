...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, bn_momentum=0.1, pretrained=False, output_stride=16, arch_name = 'resnet50'):
        if output_stride ==16:
            dilations = [1, 1, 1, 2]
            strides = [1, 2, 2, 1]
        ...
        if pretrained:
            self._load_pretrained_model(arch_name)
    ...
    def _load_pretrained_model(self, arch_name):
        pretrain_dict = model_zoo.load_url(model_urls[arch_name])
        model_dict = {}
        state_dict = self.state_dict()
        for k, v in pretrain_dict.items():
            if k in state_dict:
                model_dict[k] = v
        #dict的方法，将model_dict加入state_dict
...
def resnet50(bn_momentum=0.1, pretrained=False, output_stride=16):
    ...
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], bn_momentum, pretrained, output_stride, 'resnet50')
    return model
...
def resnet101(bn_momentum=0.1, pretrained=False, output_stride=16):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], bn_momentum, pretrained, output_stride, 'resnet101')
    return model
...
if __name__ =="__main__":
    ...
    model101 = resnet101(pretrained=True)
    model152 = resnet152(pretrained=True)
    device = torch.device('cpu')
    model50 = model50.to(device)
    model101 = model101.to(device)
    model152 = model152.to(device)
    summary(model50, (3, 513, 513), device="cpu")
    # summary(model101, (3, 513, 513), device="cpu")
    # summary(model152, (3, 513, 513), device="cpu")